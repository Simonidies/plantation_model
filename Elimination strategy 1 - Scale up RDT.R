...
## ELIMINATION STRATEGY 1: Scale-up rapid diagnostic testing ##
...
for (tau in seq(0,1,0.02)) {
  ...
  tau=tau               #** Proportion of clinical (symptomatic) patients treated. 50% seek treatment * 90% sensitivity of RDT * 90% complete treatment
    ...
    with(as.list(c(state, parameters)),
         {
           ...
           # Migration variables
           # iota <- (1+cos(2*pi*(t-phi2)/12))/12 # rate of immigration
           # eps <- (1+cos(2*pi*(t-phi3)/12))/12 # rate of emigration
           ...
           dA1 <- -mu*A1+gamma*L1-(delta/(1-p*tau))*A1-eps*M*A1/N-eon*(A1/N)*M
           dG1 <- -mu*G1+delta*A1-(zeta/(1-p*tau))*G1-eps*M*G1/N-eon*(G1/N)*M
           dR  <-  ((delta*p*tau)/(1-p*tau))*A1+(zeta/(1-p*tau))*G1+((delta*q*tau)/(1-q*tau))*A2+(zeta/(1-q*tau))*G2-(mu+lam+omega)*R-eps*M*R/N-eon*(R/N)*M
           dL2 <- -mu*L2+lam*R-gamma*L2-eps*M*L2/N-eon*(L2/N)*M
           dA2 <- -mu*A2+gamma*L2-(delta/(1-q*tau))*A2-eps*M*A2/N-eon*(A2/N)*M
           dG2 <- -mu*G2+delta*A2-(zeta/(1-q*tau))*G2-eps*M*G2/N+iota*M*(ksi)-eon*(G2/N)*M+ksi*ion*M
  ...
  elimination = rbind(elimination, data.frame(tau,(OUT$year[which(((OUT$Prevalence.pop < 0.01) & (OUT$Prevalence.pop > 0.009))== T)])[1]))
}

colnames(elimination) <- c("Tau","EliminationYears")
plot(elimination$Tau,elimination$EliminationYears, col = "dark blue",lwd=2, type='l',xlab="Tau",ylab="Yaers to Elimination", xlim= c(min=0.42, max=1), main="Years to elimination of malaria\nby proportion of symptomatic patients treated")
points(x=0.4, y=elimination$EliminationYears[which((elimination$Tau == 0.4) == T)], col = "red", pch = 19)
