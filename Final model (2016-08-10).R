...
library(ggplot2)
  ...
  gamma=1/(11/30.415)   # rate of movement from liver to asexual blood stage 
  delta=1/(10/30.415)   # rate of gametocyte maturation  
  p=(0.93)              # proportion of patients who are clinical/symptomatic(no immunity)
  q=(0.50)              # proportion of patients who are clinical/symptomatic(immunity)
  tau=0.4               #** Proportion of clinical (symptomatic) patients treated. 
  zeta=1/(120/30.415)   # rate of natural recovery from malaria infection 
  omega=1/(365/30.415)  # rate of loss of immunity 
  
  # variable parameters (force of infection)
  lagm=14/30.415  # time from mosquito biting infectious person until mosquito infectious
  ...
  initL1 <- 0            # inital liver1 stage compartment (no immunity) 
  initA1 <- 0            # inital asexual1 blood-stage compartment (no immunity)
  initG1 <- 0            # inital sexual1 gametocyte blood-stage compartment (no immunity) 
  initL2 <- 0            # inital liver2 stage compartment (with partial immunity)
  initA2 <- 0            # inital asexual2 blood-stage compartmen ( partial immunity)
  initG2 <- prev*initN   # inital gametocyte2 blood-stage compartment ( partial immunity)
  
  # initial susceptible compartment
  initS  <- initN-initL1-initA1-initG1-initR-initL2-initA2-initG2 
  
  state  <- c(S = initS, L1 = initL1, A1 = initA1, G1 = initG1, 
  	R = initR, L2=initL2, A2 = initA2, G2 = initG2)
  ...
  malaria<-function(t, state, parameters) 
  {
    ...
    parameters <- c(
      ...
      gamma=gamma,    # rate of movement from liver to asexual blood stage 
      delta=delta,    # rate of gametocyte maturation 
      p=p,            # proportion of patients who are clinical/symptomatic (no immunity)
      q=q,            # proportion of patients who are clinical/symptomatic (immunity)
      tau=tau,        # Proportion of clinical (symptomatic) patients treated
      zeta=zeta,      # rate of natural recovery from malaria infection 
      omega=omega,    # rate of loss of immunity = 1/(average duration of immunity)
      
      # variable parameters (force of infection)
      lagm=lagm,  # time from mosquito biting infectious person, until mosquito infectious
      ...
      #PCR Paramaters
      PCR_interv_start=PCR_interv_start,     # Start PCR intervention
      PCR_interv_end=PCR_interv_end,         # duration in weeks of PCR drug effect
      PCR_cov=PCR_cov,                       # coverage of PCR campaign
      PCR_eff=PCR_eff,                       # reduction in transmission:full PCR coverage
    ...
    with(as.list(c(state, parameters)),
         {
           ...
           # Define lag in Gt (G total) -- the number of people in compartments G1 and G2 
           # at the time mosquitoes were infected "lagm" weeks prior 
           #(ie. time it takes from mosquito bite until the mosquito is infectious)
           ...
           #PCR Campaign
           PCR  <- (PCR_cov*PCR_eff)*(t>(PCR_interv_start))*(t<=(PCR_interv_start+PCR_interv_end))  
           
           # rate of change
           dS  <-  nu*N-mu*S-lam*S+omega*R-eps*M*S/N+
           	iota*M*(1-ksi)-eon*(S/N)*M+(1-ksi)*ion*M
           ...
           dR  <-  ((delta*p*tau)/(1-p*tau))*A1+(zeta/(1-p*tau))*G1+((delta*q*tau)/
           	(1-q*tau))*A2+(zeta/(1-q*tau))*G2-(mu+lam+omega)*R-eps*M*R/N-eon*(R/N)*M+
           	PCR*(A1+G1)+PCR*(A2+G2)
           dL2 <- -mu*L2+lam*R-gamma*L2-eps*M*L2/N-eon*(L2/N)*M
           dA2 <- -mu*A2+gamma*L2-(delta/(1-q*tau))*A2-eps*M*A2/N-eon*(A2/N)*M-PCR*A2
           dG2 <- -mu*G2+delta*A2-(zeta/(1-q*tau))*G2-eps*M*G2/N+
           	iota*M*(ksi)-eon*(G2/N)*M+ksi*ion*M-PCR*G2
  ...
  # Percentage population prevalence (asymptomatic and symptomatic people in blood stage)
  	OUT$Prevalence.perc <- ((OUT$A1 + OUT$G1 + OUT$A2 + OUT$G2)/OUT$N)*100 
  #  Population prevalence (asymptomatic and symptomatic people in blood stage)
  	OUT$Prevalence.pop <- (OUT$A1 + OUT$G1 + OUT$A2 + OUT$G2) 
  # Monthly reported incidence = everyone in blood stages who is symptomatic and received treatment)
  	OUT$Incidence <- ((delta*p*tau)/(1-p*tau))*OUT$A1 + zeta*OUT$G1 + ((delta*q*tau)/(1-q*tau))*OUT$A2 
  #Proportion of cases with no immunity / partial immunity
  	OUT$Clinical <- (OUT$A1 + OUT$G1)/(OUT$A2 + OUT$G2) 
  OUT$year <- time/12
...
#MODEL PLOTS
  ...
  p1 <- ggplot(data=dd,aes(x=Year,y=Count,group=State,color=State)) +
    ...
    labs(title = "Compartmental model of malaria transmission", x="Year", 
    	y="Plantation inhabitants") +
  ...
  p2 <- ggplot(data=dd2,aes(x=Year,y=Count,group=State,color=State)) +
    ...
    labs(title = "Compartmental model of malaria transmission", 
    	x="Year", y="Plantation inhabitants") +
    theme(legend.title=element_blank())  +
    theme(plot.title = element_text(lineheight=.8, face="bold")) +
    theme(legend.key = element_blank())
   p2
  ...
  plot(dd4[2373:3102,4],dd4[2373:3102,2],type = 'l',
  	main="Model vs data for plantation prevalence\nin Mondulkiri Province" ,
  ...
  p5 <- ggplot(data=MMWInctab,aes(x=Year,y=Count,color="red")) +
    geom_line(size=1) +
    theme_bw()+
    labs(title = "Malaria incidence on plantations", x="Year", 
    	y="Treated symptomatic cases \nper 1000 plantation inhabitants") +
    theme(legend.title=element_blank())  +
    theme(plot.title = element_text(lineheight=.8, face="bold")) +
    theme(legend.position="none")
    p5
  
    MMWInctab$Date <- seq.Date(as.Date("01/01/07","%d/%m/%y"),
    by = "day", length.out = 3650)
    plot(MMWInctab[1095:3650,4],MMWInctab[1095:3650,2]*1/4,type = 'l', 
    	col="black",lwd=2, ylab = "Incidence per 1000 population", xlab="")
    
  # Plot 6: Phase plot
  plot(MMWInctab[,2],Prev.perc.tab[,2], type = 'l', 
  	xlab = "Incidence of symptomatic cases per 1000 plantation inhabitants", 
  	ylab = "Prevalence (%)", main = "Phase plot of incidence vs prevalance", 
  	col="red", lwd =2)
